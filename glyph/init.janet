#!/bin/env janet
(import spork :prefix "")
(import toolbox/crypto :export true)
...
(defn gen_keys []
  (let [sign_keys (crypto/sign/keygen)
        kx_keys (crypto/kx/keygen)]
...
(defn add-sign-key [pubkey]
  ...
  #(def key (base64/encode (crypto/hash/hash 32 pubkey "gnodekey")))
...
(defn id [] (slice (base64/encode (crypto/hash/hash 16 (cache/get "node/sign/public-key") "gnodekey")) 0 8))

(defn init-keys []
  (when (not (cache/get "node/sign/secret-key"))
    (let [sign-keys (crypto/sign/keygen)
          kx-keys (crypto/kx/keygen)]
